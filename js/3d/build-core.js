...
import {
  SIZE, CENTER, R_HOLE, R_BELT_LOW, R_BELT_HIGH, R_LIP,
  LEVEL_1, UNDER_1, TOP_THICK, EDGE_R, EDGE_SEG, ENTRY_X0, ENTRY_X1, SEGMENTS,
  topAt, underAt, ringPoint, lipPoint, lipOverhang, boundaryPoint, arcY,
} from './params.js';
...
function addCut(mesh, poly, color, mergeEps) {
...
export function buildCore(mesh) {
  ...
  for (let i = 0; i < SEGMENTS; i++) {
    ...
    // Внутренняя стенка пояса хранения ниже столешницы: цилиндр R1150 от пола до 760.
    addCut(mesh, [at(beltLow[0], 0), at(beltLow[1], 0), at(beltLow[1], UNDER_1), at(beltLow[0], UNDER_1)], PALETTE.belt);

    if (raised) {
      // Подъём пояса от рабочей поверхности к низу выступа, сам низ выступа 60
      // с пазом LED и его внутренняя вертикальная кромка над Sector.
      ...
      addCut(mesh, [at(lip[0], underAt(lip[0][1])), at(lip[1], underAt(lip[1][1])), at(lip[1], topLip[1]), at(lip[0], topLip[0])], PALETTE.worktop);
    }

    // Верхняя поверхность корпуса: в высокой зоне от кромки выступа, в плоской — от пояса.
    const start = raised ? lip : beltHigh;
    const startZ = raised ? topLip : [LEVEL_1, LEVEL_1];
    ...
    const point = (edge, t) => {
      const x = start[edge][0] + (outer[edge][0] - start[edge][0]) * t;
      const y = start[edge][1] + (outer[edge][1] - start[edge][1]) * t;
      return [x, y, topAt(y)];
    };
    for (let j = 0; j < 24; j++) {
    ...
    addWall(mesh, [at(outer[0], outerUnder[0]), at(outer[1], outerUnder[1]), at(outer[1], outerTop[1]), at(outer[0], outerTop[0])], PALETTE.worktop);
  ...
  for (const x of [ENTRY_X0, ENTRY_X1]) {
    ...
    if (x === ENTRY_X1) {
      addQuad(mesh, [x, yBody, 0], [x, LEDGE_Y1, 0], [x, LEDGE_Y1, LEDGE_BOTTOM], [x, yBody, LEDGE_BOTTOM], PALETTE.belt);
      addQuad(mesh, [x, LEDGE_Y1, 0], [x, SIZE, 0], [x, SIZE, UNDER_1], [x, LEDGE_Y1, UNDER_1], PALETTE.belt);
    } else {
      addQuad(mesh, [x, yBody, 0], [x, SIZE, 0], [x, SIZE, UNDER_1], [x, yBody, UNDER_1], PALETTE.belt);
    }
  ...
  addQuad(mesh, [back, wallY, 0], [back, LEDGE_Y1, 0], [back, LEDGE_Y1, LEDGE_BOTTOM], [back, wallY, LEDGE_BOTTOM], PALETTE.belt);
  addQuad(mesh, [back, LEDGE_Y1, 0], [back, SIZE, 0], [back, SIZE, UNDER_1], [back, LEDGE_Y1, UNDER_1], PALETTE.belt);
  addQuad(mesh, [ENTRY_X1, wallY, LEDGE_BOTTOM], [back, wallY, LEDGE_BOTTOM],
    [back, LEDGE_Y1, LEDGE_BOTTOM], [ENTRY_X1, LEDGE_Y1, LEDGE_BOTTOM], PALETTE.belt);
  ...
  addQuad(mesh, [ENTRY_X1, LEDGE_Y1, LEDGE_BOTTOM], [back, LEDGE_Y1, LEDGE_BOTTOM],
    [back, LEDGE_Y1, UNDER_1], [ENTRY_X1, LEDGE_Y1, UNDER_1], PALETTE.belt);
  addQuad(mesh, [ENTRY_X1, LEDGE_Y1, UNDER_1], [back, LEDGE_Y1, UNDER_1],
    [back, SIZE, UNDER_1], [ENTRY_X1, SIZE, UNDER_1], PALETTE.belt);
}
...
export function coreEdges() {
  ...
  const push = (r, z) => {
    ...
    for (let i = 0; i <= SEGMENTS; i++) {
      const a = i * step;
      const p = r === 'outer' ? boundaryPoint(a) : r === 'lip' ? lipPoint(a) : ringPoint(a, r);
